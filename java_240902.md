...
< 예외 >
```
바로 문자열을 다루는 String 인데, 초기화할 때에는
...
#### 1. 박싱
박싱은 기본 타입의 데이터를 Wrapper 클래스의 객체로 변환하는 것
```java
int index = 11;
...
### 문자열과 숫자 변환
#### 1. 문자열 → 숫자 변환
문자열에서 숫자 primitive type으로의 변환은 `parseXXX()` 메소드를 사용하여 변환하고,

문자열에서 숫자 Wrapper class로의 변환은 `valueOf()` 메소드를 사용하여 변환합니다. 
